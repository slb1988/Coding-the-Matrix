...
def vec_select(veclist, k): 
    ...
    True
    '''
    return [item for item in veclist if item[k]==0]
...
def GF2_span(D, L):
    ...
    for i in range(len(L)):
        multipliers = [item+[one0] for item in multipliers for one0 in [0, one]]
    return [sum([multiplier[i] * L[i] for i in range(len(L))]) for multiplier in multipliers]
