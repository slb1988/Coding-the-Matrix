...
def equal(u,v):
    ...
    assert u.D == v.D
    if False in [u[i]==v[i] for i in u.D]:
        return False
    return True
...
def add(u,v):
    ...
    assert u.D == v.D
    return Vec(u.D, {k:u[k]+v[k] for k in v.D})

def dot(u,v):
    "Returns the dot product of the two vectors"
    assert u.D == v.D
    return sum([u[k]*v[k] for k in v.D])

def scalar_mul(v, alpha):
    "Returns the scalar-vector product alpha times v"
    return Vec(v.D, {k:alpha*v[k] for k in v.D if alpha!=0 and v[k]!=0})

def neg(v):
    "Returns the negation of a vector"
    return Vec(v.D, {k:-v[k] for k in v.f.keys()})
